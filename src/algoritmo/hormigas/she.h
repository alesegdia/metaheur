...
namespace Hormigas
{
	...
	class SHE: public MH::Metodo<Problema>
	{
		...
		Solucion solgreedy;
		float aporte_inicial, alfa, beta, rho, factor_elite;
	...
	public:
		...
		Solucion operator()()
		{
			...
			{
				Solucion mejor_hormiga;
				mejor_hormiga.SetFitness(std::numeric_limits<float>::max());
				EvaporarFeromonas();
				for( int i = 0; i < num_hormigas; i++ )
				{
					...
					if( actual.GetFitness() < mejor_hormiga.GetFitness() )
					{
						mejor_hormiga = actual;
					}
				}
				AportarFeromonas( mejor_hormiga, factor_elite );
			}
		...
		void AportarFeromonas( const Solucion& sol, float factor=1.f )
		{
			// APORTE: aporta solo el camino
			float aporte = factor * CalcularAporte( sol.GetFitness() );
			for( int i = 1; i < Problema::PROB_SIZE; i++ )
			{
				int f1, f2;
				f1 = sol.Get(i);
